...
# 批量抓取历史数据
begin_date = '2021-01-30'  # 手工设定开始时间
end_date = '2021-01-30'  # 手工设定结束时间
date_list = []
date = pd.to_datetime(begin_date)
while date <= pd.to_datetime(end_date):
    date_list.append(str(date))  # 构造日期列表
    date += datetime.timedelta(days=1)
path = r'C:\Users\Administrator\PycharmProjects\AK_DIGICCY\data\history_candle_data'
error_list = []
for start_time in date_list:  # 按日循环
    for exchange in [ccxt.binance(), ccxt.huobipro()]:  # OKEX交易所需要特殊处理.
        symbol_list = ['BTC/USDT', 'ETH/USDT', 'EOS/USDT', 'LTC/USDT', 'ETC/USDT',
                       'XRP/USDT', 'LINK/USDT', 'DOT/USDT', 'UNI/USDT']
        for symbol in symbol_list:  # 币对循环
            if symbol.endswith('/USDT') is False:  # 只保留USDT标价的symbol
                continue
            for time_interval in ['1m']:  # K线周期循环
